    ...
    public void onForm(PlayerFormRespondedEvent e) {
        if (e.getWindow() instanceof CustomForm fw) {
            ...
            if (fw.title().startsWith("§3WorldSettings")) {
                ...
                c.set("LoadOnStart", fw.response().getResponse(0));
                c.set("Gamemode", fw.response().getDropdownResponse(1).elementId());
                c.set("fly", fw.response().getResponse(2));
                c.set("respawnworld", fw.response().getResponse(3));
                c.set("protected", fw.response().getResponse(4));
                c.set("note", fw.response().getResponse(5));
                int index = 6;
